...
function interrupter(interruptingWord) {
  ...
  return function(sentence) {
    let wordsWithInterruptions = []
    let words = sentence.split(' ')

    for (let i = 0; i < words.length - 1; i++) {
      wordsWithInterruptions.push(words[i])
      wordsWithInterruptions.push(interruptingWord)
    }
    wordsWithInterruptions.push(words[words.length - 1])
    
    return wordsWithInterruptions.join(' ')
  }
}
